...
def freeze_pretrained(model):
    ...
    for param in model.layer6.parameters(): 
        param.requires_grad = False
    for param in model.layer7.parameters(): 
        param.requires_grad = False
    for param in model.layer8.parameters(): 
        param.requires_grad = False
...
def main():
    ...
    model = EfficientNet(num_classes=10, dropout=0.1, pretrained=True).to(device)
    ...
    if args.gaussian_augm:
        ...
        mode = "adversarial"
    torch.save(model.state_dict(), f"{model_path}/efficientnet_{mode}_seed_{args.seed}.pt")
